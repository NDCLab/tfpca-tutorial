%ALG016T_ Systematic and randomized test for ALG016M_ and ALG016C_. 

%   Copyright (c) 1998 by Robert M. Nickel
%   $Revision: 1.1.1.1 $
%   $Date: 2001/03/05 09:09:36 $

% define the kernel flag
KF=0;
...
for C=0:1;   % real complex/flag
   ...
   % individual check of errors
   % [ N C L K H M w D ec ]
   %N=3; C=1; L=1; K=1; H=4; M=1; w=3; D=1;
   ...
   disp(['SYSTEMATIC TEST: (',num2str(cicl),')']);
	...
	disp(num2str([ N C L K H M w D KF ]));

	% run both algorithms
	[Am,ef]=alg016m_(S,L,K,H,M,W,D,timer,monhan,0,KF,' ');
   [Ac,ef]=alg016c_(S,L,K,H,M,W,D,timer,monhan,0,KF);
   ...
   if ec>0;
      ...
      ecdm=[ ecdm ; [ N C L K H M w D ec er KF ]];
      disp(Am); disp(Ac);
      pause(1);
      save ecdm.mat ecdm
   end
...
KF=round(0-0.49+(5-0+0.98)*rand(1));    % kernel flag
...
C=round(rand(1));                       % real complex/flag
   ...
   disp(['RANDOMIZED TEST: (',num2str(cicl),')']);
	disp('N  C  L  K  H  M  w  D  kernel');
	disp(num2str([ N C L K H M w D KF ])); drawnow;
...
%%%%%%%%%%%%%%%%%
   ...
   [Am,ef]=alg016m_(S,L,K,H,M,W,D,1,monhan,1,KF,' ');
   ...
   [Am,ef]=alg016m_(S,L,K,H,M,W,D,1,monhan,0,KF,' ');
   tofl=job_mon('done',monhan);
   disp(['Return Status: ',num2str(ef)]);
...
%%%%%%%%%%%%%%%%%
   ...
   % while counting loops
   monhan=job_mon('Local ACF Computation',tica);
   [Am,ef]=alg016c_(S,L,K,H,M,W,D,1,monhan,1,KF);
   pause(5);
   tofl=job_mon('done',monhan);
   disp(['Return Status: ',num2str(ef)]);
   pause(1);
   ...
   % while counting flops
   monhan=job_mon('Local ACF Computation',tofl);
   [Am,ef]=alg016c_(S,L,K,H,M,W,D,1,monhan,0,KF);
   pause(5);
   tofl=job_mon('done',monhan);
   disp(['Return Status: ',num2str(ef)]);
