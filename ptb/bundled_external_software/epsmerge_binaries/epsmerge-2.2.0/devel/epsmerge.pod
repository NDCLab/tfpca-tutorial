...
There I<must> be a space separating the option from its value.  The value
...
Causes B<epsmerge> to print out a usage message, and then stop.
...
Like B<-x>, but how many cells you want in each column.  You may specify
either, both, or none, of B<-x> and B<-y>; if you specify none then the
...
its labels I<above> the EPS files.
