...
The TFPCA-Tutorial aims for serving as a jumping off point for more advanced analyses leveraging Cohen's class RID and/or TF-PCA. It consists of template scripts to showcase how to :
1) prepare data for psychophysiology toolbox (ptb);
2) implement Cohen's class reduced interference distribution (RID) on the data to compute both average and total power; 
3) apply principle components analysis (PCA) to the resulting time-frequency surfaces; 
4) convert the resulting data generated by ptb into a format that is easy-to-understand/plotting/intended analysis;
5) template plotting scripts.
