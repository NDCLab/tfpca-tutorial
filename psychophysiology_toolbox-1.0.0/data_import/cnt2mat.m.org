...
% EVENTS 
  ...
  % make EVENT vector 
    ...
    for k = 1:length(event.Offset),
      eventvect(event.Offset(k)) = event.StimType(k);
    end

%   % old method - failed to pull all triggers if any Offset repeats  
%   %              (This was a Neuroscan bug that sometimes happened with synamps 2 and Scan 4.3)  
%   eventvect = int16(ones(1,head.nsamples) * -1);
%   testvect = zeros(1,head.nsamples); 
%   curevent = 1; 
%   for k = 1:head.nsamples,  
%     testvect(k) = k;  
%     if event.Offset(curevent) == k, 
%       eventvect(k) = event.StimType(curevent); 
%       if curevent==nevent, disp(['break']); break; end  
%       curevent=curevent+1;  
%     end 
%   end 
