...
% 
% NOTE: want to modify to bring event info into EEG.epoch, in addition to EEG.event as is now 
%  
...
% epochs/events 
  ...
  if isfield(erp,'subnum'),
    for jj=1:EEG.trials,  
      ...
      EEG.event(event_counter).type      = 'subnum';
      ...
      EEG.event(event_counter).epochtype = erp_epochs.subnum(jj); 
    end
  end     
  if isfield(erp,'subs'), 
    for jj=1:EEG.trials,  
      event_counter = event_counter + 1;    
      EEG.event(event_counter).epoch     = jj;
      EEG.event(event_counter).type      = 'subname'; 
      EEG.event(event_counter).latency   = (jj-1) * EEG.pnts + erp.tbin;  
      EEG.event(event_counter).duration  = 0;
      EEG.event(event_counter).accept    = 1; 
      EEG.event(event_counter).response  = 0; 
      EEG.event(event_counter).epochtype = char(erp.subs.name(erp_epochs.subnum(jj),:)); 
    end
  end  
  for jj=1:EEG.trials,
      event_counter = event_counter + 1;
      EEG.event(event_counter).epoch     = jj;
      EEG.event(event_counter).type      = 'trigger';
      EEG.event(event_counter).latency   = (jj-1) * EEG.pnts + erp.tbin;  
      EEG.event(event_counter).duration  = 0;
      EEG.event(event_counter).accept    = erp_epochs.accept(jj);
      EEG.event(event_counter).response  = 0;
      EEG.event(event_counter).epochtype = erp_epochs.ttype(jj);
  end
  if isfield(erp,'stim'), 
    ...
    for fn=1:length(fns),  
      for jj=1:EEG.trials,  
        event_counter = event_counter + 1;    
        EEG.event(event_counter).epoch     = jj;
        EEG.event(event_counter).type      = char(fns(fn));
        EEG.event(event_counter).latency   = (jj-1) * EEG.pnts + erp.tbin;  
        EEG.event(event_counter).duration  = 0;
        EEG.event(event_counter).accept    = 1; 
        EEG.event(event_counter).response  = 0; 
        if isnumeric(eval(['erp.stim.' char(fns(fn)) ])), 
          EEG.event(event_counter).epochtype =      eval(['erp_epochs.stim.' char(fns(fn)) '(jj);' ]); 
        else, 
          EEG.event(event_counter).epochtype = char(eval(['erp_epochs.stim.' char(fns(fn)) '(jj);']));   
        end  
      end
    end 
  end 
