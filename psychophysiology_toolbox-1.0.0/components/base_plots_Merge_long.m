function [mergename,retval] = base_plots_Merge_PCdiffs_long(IDvars,SETvars),
% [mergename,retval] = base_plots_Merge_PCdiffs_long(IDvars),
%  
...
% difference/stats plots by main effect 
  if isfield(SETvars,'comparisons')==1, 
...
% epsmerge
  if     numofeffects+1 > 12, 
    numofcols = 9;
  elseif numofeffects+1 >  3, 
    numofcols = 6; 
  else, 
    numofcols = 3; 
  end 
  numofrows = ceil(((numofeffects+1)*3)/numofcols);
