...
%     if isstruct(SETvars.trl2avg.domain), 
%       switch SETvars.trl2avg.domain.domain,
%         case 'time', domain = 'time';
%         case {'freq-power','freq-amplitude'}, domain = 'freq';
...
%   end  
   ...
   %if isfield(erp,'domain'),               domain = erp.domain; end 
    if ~isfield(erp,'domain'),              erp.domain = 'time'; end 
    domain = erp.domain; 
    if (exist('ISFspecs','var') && isfield(ISFspecs,'domain')),
      if isstruct(ISFspecs.domain),         domain = ISFspecs.domain.domain;
      else,                                 domain = ISFspecs.domain; end
    end
    if (isfield(SETvars,'trl2avg') &&  isfield(SETvars.trl2avg,'domain')), 
      if isstruct(SETvars.trl2avg.domain),  domain = SETvars.trl2avg.domain.domain; 
      else,                                 domain = SETvars.trl2avg.domain; end 
    end 
    switch domain
      case 'time',%domain = 'time';
      case {'freq-power','freq-amplitude'}, domain = 'freq';
      case 'TFD', disp(['ERROR: input data cannot be TFD, must be time or freq']); return; end
  ...
  % parse electrode variables 
    ...
    % electrode_locations 
    if isfield(SETvars,'electrode_locations') && ~isequal(SETvars.electrode_locations,'none'),
    ...
    % handle ISFspecs and trl2avg 
        ...
        % ISFspecs 
          if exist('ISFspecs','var') && isfield(ISFspecs,'subnames'),
            ...
            if ~isfield(ISFspecs,'OPTIONS'), 
              if isfield(SETvars,'trl2avg') && isfield(SETvars.trl2avg,'OPTIONS'),
                                                      ISFspecs.OPTIONS          = SETvars.trl2avg.OPTIONS;
              else,                                   ISFspecs.OPTIONS          =                 0; end,  
            else,  
              if isfield(SETvars,'trl2avg') && isfield(SETvars.trl2avg,'OPTIONS'),
                                                      sout = SETvars.trl2avg.OPTIONS;
                                                      for f = fieldnames(ISFspecs.OPTIONS)'
                                                          sout = setfield(sout,f{1},getfield(ISFspecs.OPTIONS,f{1}));
                                                      end 
                                                      ISFspecs.OPTIONS = sout; clear sout  
              end 
            end  
            ...
            if ~isfield(ISFspecs,'domain'),           ISFspecs.domain           =         erp.domain; 
        ...
        % SETvars.trl2avg 
          if isfield(SETvars,'trl2avg'),
            ...
            if ~isfield(SETvars.trl2avg,'OPTIONS')
              if isfield(ISFspecs,'OPTIONS'),
                                                                SETvars.trl2avg.OPTIONS             = ISFspecs.OPTIONS;
              else,                                             SETvars.trl2avg.OPTIONS             =          0; end, 
            else, 
              if isfield(ISFspecs,'OPTIONS'),
                                                                sout = ISFspecs.OPTIONS;
                                                                for f = fieldnames(SETvars.trl2avg.OPTIONS)'
                                                                    sout = setfield(sout,f{1},getfield(SETvars.trl2avg.OPTIONS,f{1}));
                                                                end
                                                                SETvars.trl2avg.OPTIONS = sout; clear sout
              end 
            end  
            ...
            if ~isfield(SETvars.trl2avg,'domain'),
              if exist('ISFspecs','var') && isfield(ISFspecs,'domain'), 
                                                                SETvars.trl2avg.domain              = ISFspecs.domain;
              else,                                             SETvars.trl2avg.domain              = erp.domain;
             %elseif isfield(erp,'domain'),                     SETvars.trl2avg.domain              = erp.domain;
