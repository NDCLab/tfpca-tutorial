...
B<epsmerge> [I<option> I<value>] [B<-->] I<file-1> I<file-2> ... 

=head1 DESCRIPTION

B<epsmerge> is a program that merges several EPS files into one
big EPS file.  The original files are untouched.  (Unless your output
file is also one of the input files; don't do that :-)

B<epsmerge> is provided AS IS copyrighted under the GNU GPL.
Be sure to read the copyright section if you do not know what this
means.

=head1 OPTIONS

An I<option> is a string telling epsmerge how to behave; each option
begins with '-' (called short options) or '--' (called long options).
Usually, an option should be followed by a single I<value> (could be a
...
There I<must> be a space or a '=' separating the option from its value.
The value may not contain whitespaces (unless the entire value is
quoted).

Short options are case sensitive (i.e., B<-O> is not equivalent to B<-o>),
but the long ones are case insensitive (B<--orientation> is equivalent to
B<--orIeNtAtIon>).

Note that if you I<don't> specify an option on the command line when
...
you give the B<-?> option to epsmerge.

Note also that a B<--> (two dashes) by itself signifies the end of the
...
B<epsmerge> currently understands the following options:

=head2 Options: Version and help
...
Causes B<epsmerge> to allege a hopefully useful arranged abridged
...
Causes B<epsmerge> to print a version string (one line) and then
stop.

=head2 Options: general options
...
section L<How to write labels> for how to specify the header string.

B<-u>

B<--labels>

Use this option to print labels for each EPS file.  See section
L<How to write labels> for how to specify the label string.

B<-o>

B<--output-file>

This option tells B<epsmerge> to write its output to the specified file.
Otherwise, the default is to write to standard output.  Which you can then
pipe on to your favourite printer (but you may want to see L<Examples of usage>
below).  And the B<--print> switch.  However, if you generate multiple files
(see L<How to generate multiple files>) then the filename you specify
will only be used to I<derive> the names of the output files.
...
Landscape.  Equivalent to B<--orientation=L>.

B<-R>

Portrait.  Equivalent to B<--orientation=P>.

B<-p>

B<--paper>

Paper specifies the paper size.  Normally for an EPS file the size of
...
Specifies the height of the paper directly.  Should be used with B<-pw>.
...
Specifies the width of the paper directly.  Should be used with B<-ph>.
...
several pages, so if you format more than one page (see section L<How to
generate multiple files>), you will get them all in one postscript file.
This option automatically implies B<--print>.
...
label for you, one for each label specifier (see L<How to write labels>)
that contains the specifier letter B<s>.  The script should return a one
line string (either as returned by the last statement, or by an explicit
I<return> statement).  If you do not specify a script on the command
...
specify both B<--header> and B<--labels> then the header label will be
...
specify, say, B<sss> as the value to B<--labels>, then your script will
be called three times for each label, with $l equal to, respectively, 0,
1, and 2 for each call.  See L<Examples of usage>.  Oh, and be sure that
your script is enclosed in quotes if typed on the command line, just as
you would when running B<perl -e> (in the configuration file just type
the script as any normal perl program (block)).

B<--ignore-eof>

This option causes the reader to ignore the %%EOF DSC which normally marks
the end of a postscript file.  Default is to use them.

=head2 Options: formatting options, standard formatter

These options are the ones understood by the default formatter.  This
formatter places things in rows and columns of I<cells>.  Imagine each
page as a matrix made of cells each of the same size, and normally
(modulo the B<-prs> and B<-par> options, see below) images are formatted
to fit each into one cell (in the order they are given on the command
line, but see also the B<-rmo> option).
...
The number following the B<-x> switch specifies how many cells you want
...
Like B<-x>, but how many cells you want in each column, i.e. how many
rows you want.  You may specify either, both, or none, of B<-x> and
B<-y>; if you specify none then the formatter will make an (educated)
guess.
...
The B<--margin> option sets a default margin (its default is, in turn,
...
Y Cell Spacing.  Analogous to the B<-xcs> option (qv).

B<-rmo>

Row major order.  If true (this is the default, too) then the cells will
be set with I<rows> first (starting with the first EPS file you
specified, then the next, etc), filling out the top row first.  If the
value is false, then the I<columns> will be set one at a time, starting
with the leftmost.

B<--major>

Must be either B<--major=rows> (the default) or B<--major=columns>.  The
former is equivalent to B<-rmo=true>, and the latter equivalent to
B<-rmo=false>.
...
=head2 Formatting options, stacking formatter
...
specify both B<-x> and B<-y> (one of which should be 1), e.g.,
...
The moral equivalent of B<-x> (qv).
...
=head1 How to write labels

Two options, namely the B<--header> and B<--labels> options, allow you
...
by default B<--labels> writes labels I<below> its EPS file, and B<--header> writes
its labels I<above> the EPS files (i.e., on top of the page).
...
the B<--header> option, this just means the time when epsmerge is run,
written in standard (UNIX) time format.  For the B<--labels> option,
...
For the generated file, if you haven't given a filename (via the B<-o> option),
`stdout' is written.

B<F>:
The letter `F' does the same as `f' except that the file's extension (if present)
is stripped.  So if your file is called F<foo.eps>, only F<foo> is written.
...
the B<--script> option (qv), but if you don't, epsmerge will prompt you
...
EPS file's DSC -- if not, it's ignored).

Each of these letters gives you one I<line> of labelling (or, if you are
from the US, labeling).

If you omit I<all> of these letters, then some default is assumed.

=item 3

As the final information in the label string you may write a number, specifying the
required font size.  The default for B<--labels> is 12, the default for B<--header>
is 20.
...
A line consisting of I<option>=I<value> or I<option>: I<value>.  The
...
write it on the command line (except that there I<must> be a value; for
...
but you have forgotten about some B<o=> in your config file, then
epsmerge will write to the B<o> file, leaving B<new_file.eps> empty.  In
this case, you should use the B<-o> switch.
...
postscript file you want to use.  See section L<Using Postscript> below.

=back

These can be configured by editing the epsmerge file F<epsmerge>; look
for the section `User serviceable parts'.

=head1 How to generate multiple files

epsmerge can print multiple files as follows: if you call epsmerge with
something like

B<epsmerge> B<-x> I<x> B<-y> I<y> B<-o> foo.eps filea.eps fileb.eps ...

where the number of input files is greater than the product of I<x> and
I<y>, then epsmerge will generate several output files.  In the first
file, epsmerge will format the first I<xy> input files; the second
output file will contain the next I<xy> input files, etc.  (As usual,
...
formatted with I<exactly> the same options.

The filenames of the output files will be generated by the given
filename (in the example B<foo.eps>) as follows: If less than 10 files
are generated, then they will be named B<foo-0.eps>, B<foo-1.eps>, etc.
If 10 or more but less than 100 files are generated, then they will be
named B<foo-00.eps>, B<foo-01.eps>, etc.  Thus the output files are
generated in alphabetical order.  And are thus easier to handle
wholesale by the shell as B<foo-??.eps>.

Alternatively, if you use the B<--postscript> option, you will get a
single postscript file with I<xy> EPS files formatted on each page.

Unless specified otherwise, this will be decided by looking at the
file's extension: if the file is called B<foo.eps> you get multi-file
EPS'es; if the file is called B<foo.ps> you get single-file multi-page
...
in your path.  See L<The Path> below.
...
or let the shell do the job (remember: the shell is your I<friend>!)
...
Second method uses B<epsmerge> to create temporary files:
...
(Note the single quotes preventing the UNIX shell from messing with the
...
where ?.eps is your favourite selection of simple EPS files.  Note that
...
This will generate the ouput files B<page-0.eps> (containing the first four
input files), B<page-1.eps> (containing the next four), and B<page-2.eps>
...
a postscript file, i.e., if the file F<warp-drive.ps> contains eight
pages, then it will get expanded as eight pages in your input.  Example:

	epsmerge -o foo.ps -x 2 -y 2 warp-drive.ps

will take the pages of the file F<warp-drive.ps> and reformat them
...
by two) images on each page; the first image will be F<bar.eps>, the
next three images on the first page will be pages 3, 1, and 5 of
F<eek.ps>.  The second page will contain pages 6, 7, 8, and 9 of
F<eek.ps>.  The third and final page will contain page 4 of F<eek.ps>
and F<moo.eps>.
...
there is I<no space> anywhere.  The format is I<filename> followed by the
I<separator> followed by a sequence of comma-separated numbers in which a dash
between two numbers I<5-9> is interpreted as the range I<5,6,7,8,9>.  If the
...
The above example assumes that the I<separator> is the single character
`:'.  However, this might have been changed when epsmerge was installed (see
section L<Further customization> for details).  Type
...
B<epsmerge> is distributed as several files: the main file and several
files with .pm extension (Perl modules).  If you use B<epsmerge> often, you may
...
assuming you unpacked epsmerge in the directory /home/foo/epsm and that F<perl> is
in /usr/bin.  Make the script executable (with F<chmod>) and put it in your path.
If the script is called F<epsmerge> then you can go to your favourite EPS directory
and merge away by just writing F<epsmerge> (plus all the options and filenames).

=head1 Terminology and abbreviations

B<Bounding Box>: Part of Adobe's DSC (qv).  As far as epsmerge is concerned,
this is the most important part of any (encapsulated or not) postscript file.

B<DSC>: Document Structuring Conventions.  Adobe's DSC specify how an EPS file
is formatted by requiring certain comments, all starting with %% and then a
keyword.  Example: '%%BoundingBox: 0 0 485 384'.

B<EPS>: Encapsulated Postscript.  This is a file that contains postscript commands
...
Observe that, unlike postscript files, an EPS file can only be I<one> page.

B<Perl>: Practical Extraction and Report Language.  Or as its many fans will
have it: Pathologically Eclectic Rubbish Lister.  Chosen for portability; although
developed for a UNIX (Linux) system, B<epsmerge> I<might> run happily on many
other platforms (currently it doesn't, though).  Oh, and Perl is I<fun>, too.
See
http://www.perl.com/
...
=head1 Warranty and Copyright

B<epsmerge> is distributed under the GNU General Public Licence:
...
that this editor can also somehow produce EPS files.  epsmerge does not
...
B<a2ps>, the all-to-postscript program at
http://www-inf.enst.fr/~demaille/a2ps/

B<psutils>, Angus Duggan's postscript utility programs; in particular,
there is a program for putting several postscript I<pages> into one page
(B<psnup>), and also a program for fitting one EPS file into a
constrained box (B<epsffit>), and B<psmerge>, a program for merging
several postscript documents (created with the same application) into
one document.

=head1 Trademarks

B<Postscript> is a trademark of Adobe Systems Inc.
