...
%
%  2. Regression has three EOG channels: left vertical, horizontal, right vertical:
%
%    selection_cards =  {'1 2','3 4', '5 6','7 8'};
%    EEG = gratton_emcp(EEG,selection_cards,{'LVEOGUP','LVEOGLO'},{'HEOGL','HEOGR'},{'RVEOGUP','RVEOGLO'});
%
%  3. First channel in the regression is vertical EOG consisting of the average of left and right vertical channels:
%
%    selection_cards =  {'1 2','3 4', '5 6','7 8'};
%    EEG = gratton_emcp(EEG,selection_cards,{'LVEOGUP RVEOGUP','LVEOGLO REOGLO'},{'HEOGL','HEOGR'});
%
%  4. Correct for a single EOG channel (must be a bipolar vertical channel, corrects only blinks and vertical EOG)
%
%    selection_cards =  {'1 2','3 4', '5 6','7 8'};
...
%         55, 468-484.  (Reports the original algorithm for one channel of vertical EOG).
%
...
%         (Reports a distribution of this program for the RT11 operating system FORTRAN IV
%          that corrects for vertical EOG and horizontal EOG.)
%
%   Derived from the original C code developed at the University of Illinois Cognitive
%   Psychophysiology Laboratory, available at http://www.umich.edu/~wgehring/emcp2001.zip.
%
%  Notes:
%
%  What is a selection card?  Selection cards (yes, they used to be paper cards)
%  identify epochs according to time-locking events (i.e., EEG.epoch.eventtypes
%  that have latency of 0). Epochs identified by a selection card are used to
%  create an average waveform, and those averages are subtracted from each single trial
%  before correction factors are computed.  The aim of this is so that the correction
%  factors are not affected by event-related activity (real ERPs).  In practice, you
%  want to choose averages that will differ meaningfully, so if you have an
...
%  and 2.  Before the regression is performed, all EEG epochs with trial type
%  1 will have the frequent average subtracted.
%
...
%
%  1) The EEG dataset must have two channels that can be used to derive
%  vertical EOG and two channels that can be used to derive horizontal EOG
%
...
%
%  In the case where each epoch matches one selection card,
%  the output from this function should match the emcpx C code output
%  from the Cognitive Psychophysiology Laboratory at Illinois.
%  (see http://www.umich.edu/~wgehring/emcp2001.zip).
%
%  "Trash data" are data epochs that do not match one of the selection cards. All
%  epochs in the data file are corrected, even those that don't match a
%  selection card. If there are such trash data in the input EEG data, the output
%  of this function will be slightly different from the Illinois C program as
%  the C code used trash data epochs to compute the variance but left
%  the trash data out of other computations.  Greg Miller's FORTRAN code
%  (which more closely corresponds to Gratton's original FORTRAN code) used the
%  trash data in all steps of computing the regression. Thus, it is unclear whether
%  the use of the trash data in the C code for one step of the procedure but
%  not for others was a desired feature.  In any case, the trash data play
%  no role in computing the correction factors in the present function. Modern
%  epoched datasets can be time-locked to any event (e.g., stimulus, response), so
%  computing a single average across such hetereogeneous events and subtracting it from
%  single trials does not make sense. Leaving the trash data out also
%  allows the desirable feature in which certain data are excluded from computing
%  the correction factors.
%
%  The function does not do the following things that the original code did:
%
%  1.  The original Gratton code checked for data out of scale and flat lines and
%      discarded those trials.
%
%  2.  In some cases it tried to reconstruct the blink on the VEOG channel when
%      data on the channel went out of scale.
%
%      Because of these changes, artifact rejection and other data screening should
%      take place prior to emcp, because the procedure is sensitive to bad data.
%
function EEG = gratton_emcp_mult2(EEG, selection_cards, varargin)
%
tic;
starttime = cputime;
if nargin < 3
...
markBlink =  logical(zeros(size(EEG.data,2), size(EEG.data,3)));  %points x trials
covariance = zeros(EEG.pnts,EEG.trials);
for pt = initBlinkScan:endBlinkScan
    start1 = pt - initBlinkScan+1;
    end1 = start1 + thirdOfBlink;
    start2 = end1 + 1;
    end2 = start1 + 2 * thirdOfBlink;
    start3 = end2+1;
    end3 = pt + initBlinkScan-1;
...
xvecS = [];
yvec = [];
for ch = 1:length(chanpair)
    xvecB = [xvecB subdata(ch,rmarkBlink)'];
    xvecS = [xvecS subdata(ch,~rmarkBlink)'];
end
for ch = 1:EEG.nbchan+length(chanpair)
...
EEG.emcp.table = [EEG.emcp.table; sprintf(blanks(tablewidth))];
EEG.emcp.table = [EEG.emcp.table; [strjust(sprintf(['%' num2str(tablewidth) 's'],'Propagation'),'center')]];
EEG.emcp.table = [EEG.emcp.table; [strjust(sprintf(['%' num2str(tablewidth) 's'],'Factors'),'center')]];
EEG.emcp.table = [EEG.emcp.table; sprintf(blanks(tablewidth))];
for i=1:length(chanpair)
