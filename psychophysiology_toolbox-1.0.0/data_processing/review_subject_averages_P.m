...
%[erp] = review_subject_averages(erp_inname,outname,electrode_montage,filterspec,AT,verbose,starttrial)
% 
...
% vars 
  ...
  if ~isfield(PARAMETERS,'outname'),           outname           ='';
  ...
  if ~isfield(PARAMETERS,'starttrial'),        starttrial        =1;
  else,                                        starttrial        =PARAMETERS.starttrial;        end
  if ~isfield(PARAMETERS,'catcodes'),          catcodes          =0;
  else,                                        catcodes          =PARAMETERS.catcodes;          end
        ...
        if exit_flag==1, 
          ...
          % loop - plot and ask for changes/corrections 
              while 1,
                  ...
                  % plot 
                  %catcodes(1).name = usweep((t)); catcodes(1).text = ['erp.subnum==' num2str(usweep((t))) ];  
                  erp_temp=reduce_erp(erp,['erp.subnum==' num2str(usweep(t)) ]); 
                  plot_erpdata(erp_temp,'','','mean',electrode_montage,catcodes,0,0,0,0,0,0,0,'none',1); 
                  suptitle(['Subname: ' char(erp.subs.name(usweep(t),:)) ' -- Accept=' num2str(mean(erp.accept(erp.subnum==usweep(t)))) ]); 
                  ...
                  % accept change 
                  if accept_change==1, 
                    switch mean(erp.accept(erp.subnum==usweep(t))) 
                    case 1,  
                      erp.accept(erp.subnum==usweep(t)) = 0; 
                    case 0, 
                      erp.accept(erp.subnum==usweep(t)) = 1; 
                    end 
                  end 
